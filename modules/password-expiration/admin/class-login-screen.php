...
final class Login_Screen {
	...
	public function lost_password_message( $message ) {
		...
		return sprintf(
			...
			sprintf(
				// translators: %d: day.
				_n( 'Your password must be reset every %d day.', 'Your password must be reset every %d days.', $limit, 'asvz' ),
				$limit
			),
			esc_html__( 'Please enter your username or e-mail below and a password reset link will be sent to you.', 'asvz' )
		);
