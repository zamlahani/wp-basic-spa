	...
	function setupData() {
		...
		if (asvzObj.tasks.length) {
			asvzObj.tasks.forEach(function (el, i) {
				var categorySlug = asvzObj.categories.find(function (val, i, arr) { return val.term_id == el.category }).slug
				asvzObj.searchData.push({
					name: el.post_title,
					slug: el.post_name,
					id: el.ID,
	...
	function setupEvents() {
		$(".autocomplete").on('select:flexdatalist', function (e, o, opts) {
			...
			if (o.type === 'category') {
				...
				window.location.href = `/#/category/${o.categorySlug}/${o.slug}`;
			} else if (o.type === 'step') {
				task = asvzObj.tasks.find(function (item, i) {
					return item.ID === o.task_id;
				});

				categorySlug = asvzObj.categories.find(function (val, i, arr) { return val.term_id == task.category }).slug

				if (task) {
					window.location.href = `/#/category/${categorySlug}/${task.post_name}`;
				}
			} else if (o.type === 'material') {
				window.location.href = `/#/product/${o.slug}`;
			} else if (o.type === 'faq') {
